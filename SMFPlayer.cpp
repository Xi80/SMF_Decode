...
int SMFPlayer::Open(const char* fileName)
{
	...
	/* マジックワードをチェックする */
	if (get4(&magic) != SMF_RET_OK || magic != 0x4D546864)return SMF_RET_ERR_WROND_MAGIC;

	/* データ長さ取得 */
	if (get4(&chunkLength) != SMF_RET_OK || chunkLength != 6)return SMF_RET_ERR_WRONG_LENGTH;

	/* フォーマット取得 */
	if (get2(&format) != SMF_RET_OK || format != 0)return SMF_RET_ERR_WRONG_FORMAT;

	/* トラック数取得 */
	if (get2(&trackCount) != SMF_RET_OK || trackCount != 1)return SMF_RET_ERR_WRONG_COUNT;

	/* 分解能取得 */
	if (get2(&resolution) != SMF_RET_OK || resolution == 0)return SMF_RET_ERR_WRONG_RESOLUTION;
	...
	/* トラックに移動する */
	if (get4(&magic) != SMF_RET_OK || magic != 0x4D54726B)return SMF_RET_ERR_TRACK_NOT_FOUND;

	/* データ長さ取得 */
	if (get4(&chunkLength) != SMF_RET_OK || chunkLength == 0)return SMF_RET_ERR_WRONG_LENGTH;
...
int SMFPlayer::Play(uint32_t* nextDelta)
{
	size_t size;
	if (getMessage(message, &size) != SMF_RET_OK)return SMF_RET_ERR_REACHED_EOF;
	if (getVarLen(&delta) != SMF_RET_OK)return SMF_RET_ERR_REACHED_EOF;
	midiMessageCallback(message, size);
	*nextDelta = delta * deltaBase / 1000;
	return SMF_RET_OK;
}
